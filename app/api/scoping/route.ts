...
export async function POST(request: NextRequest) {
  try {
    ...
    // Ensure date is null if empty string
    const targetDeploymentDate = data.targetDeploymentDate || null
    ...
    const questionnaire = await sql`
      INSERT INTO scoping_questionnaire (
        ...
        ${targetDeploymentDate}, ${data.budgetRange}, ${data.projectTimelineMonths},
    ...
    return NextResponse.json({ error: "Failed to create questionnaire", details: errorMessage }, { status: 500 })
  }
}
