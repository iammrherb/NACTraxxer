...
export async function POST(request: NextRequest) {
  try {
    ...
    INSERT INTO scoping_questionnaire (
      ...
      ${data.totalSites}, ${data.totalUsers}, ${data.networkVendors}::jsonb, 
      ${data.switchModels}::jsonb, ${data.wirelessModels}::jsonb,
      ${data.identityProviders}::jsonb, ${data.mdmSolutions}::jsonb,
      ${data.siemSolutions}::jsonb, ${data.mdrSolutions}::jsonb,
      ${data.xdrSolutions}::jsonb, ${data.edrSolutions}::jsonb,
      ${data.authMethods}::jsonb, ${data.certificateRequirements}::jsonb,
      ${data.guestAccessRequirements}::jsonb, ${data.byodRequirements}::jsonb,
      ${data.vlanSegmentation}::jsonb, ${data.accessPolicies}::jsonb,
      ${data.complianceRequirements}::jsonb, ${data.riskTolerance},
      ${data.securityPriorities}::jsonb, ${data.complianceFrameworks}::jsonb,
