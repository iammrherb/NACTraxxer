...
import { Cloud, Network, Shield, Settings, Zap, Globe, Lock, Users, Server, Download } from "lucide-react"
...
export default function ArchitectureDesigner() {
  ...
  const exportAsSVG = async (svgElement: SVGElement) => {
    ...
    downloadFile(blob, `portnox-architecture-${view}-${Date.now()}.svg`)
  }
  ...
  const exportAsPNG = async (svgElement: SVGElement) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          downloadFile(blob, `portnox-architecture-${view}-${Date.now()}.png`)
        }
      }, "image/png")
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={view} onValueChange={setView}>
                ...
                <SelectContent>
                  {architectureViews.map((viewItem) => (
                    <SelectItem key={viewItem.id} value={viewItem.id}>
      ...
      <div className="space-y-4">
        ...
        />
        <ArchitectureLegend currentView={view} />
      </div>
    </div>
  )
}
