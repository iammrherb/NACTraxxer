...
export default function RolloutProgress() {
  ...
  const loadRolloutData = async () => {
    setLoading(true)
    try {
      console.log("Loading rollout data...")

      // Load from storage
      const storedSites = await storage.getSites()
      const storedEvents = await storage.getEvents()

      console.log(`Loaded ${storedSites.length} sites and ${storedEvents.length} events`)
      ...
      if (storedSites.length > 0) {
        ...
        console.log("No stored data found, generating demo data...")
  ...
  const generateRolloutFromData = async (sitesData: any[], eventsData: any[]) => {
    ...
    console.log("Generating rollout data from stored sites and events...")
    ...
    console.log(`Generated ${generatedPhases.length} phases and ${generatedSites.length} sites for rollout tracking`)
  }

  const generateDemoRolloutData = async () => {
    console.log("Generating demo rollout data...")
    const currentDate = new Date()
    ...
    console.log(`Generated ${demoPhases.length} demo phases and ${demoSites.length} demo sites`)
  }
  ...
  const handleRefresh = () => {
    ...
    toast({
      ...
      description: "Rollout progress data has been refreshed.",
    })
  }
