...
import { Download, Settings, Eye, Layers, Network, Shield, Cloud, Building, Wifi, Server, Lock, Key, Users, Monitor, Smartphone, Laptop, Printer, Camera, FileImage, FileText } from 'lucide-react'
...
export default function ArchitectureDesigner() {
  ...
  const exportDiagram = async (format: 'svg' | 'png' | 'pdf') => {
    try {
      ...
      titleElement.textContent = `ABM Industries - Zero Trust NAC Architecture (${config.cloudProvider.toUpperCase()})`
      
      const subtitleElement = document.createElementNS('http://www.w3.org/2000/svg', 'text')
      subtitleElement.setAttribute('x', '20')
      ...
      subtitleElement.textContent = `Generated on ${new Date().toLocaleDateString()} | View: ${selectedView}`
      
      clonedSvg.insertBefore(titleElement, clonedSvg.firstChild)
      clonedSvg.insertBefore(subtitleElement, clonedSvg.children[1])
