...
import type { LibraryData, Vendor, ScopingQuestionnaire as ScopingQuestionnaireType } from "@/lib/types"
...
interface CheckboxGridProps {
  title: string
  options: (Vendor | { id: number | string; name: string })[] | undefined
  selected: string[]
  onSelectionChange: (newSelection: string[]) => void
}

const initialFormData: ScopingQuestionnaireType = {
  organizationName: "",
  totalUsers: 1000,
  siteCount: 1,
...
export function ScopingQuestionnaire({ isOpen, onClose, onSave, library, questionnaire }: ScopingQuestionnaireProps) {
  const [formData, setFormData] = useState<ScopingQuestionnaireType>(initialFormData)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl">
        ...
        <DialogFooter className="justify-between pt-4">
          ...
          <div>
            <Button variant="secondary" onClick={() => handleSubmit("Draft")} disabled={!library}>
              Save as Draft
            </Button>
            <Button onClick={() => handleSubmit("Completed")} className="ml-2" disabled={!library}>
