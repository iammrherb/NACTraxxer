...
export function SiteDetailModal({ isOpen, onClose, site, onUpdate, library, users }: SiteDetailModalProps) {
  ...
  const handleTestCaseStatusChange = (testCaseId: string, newStatus: string) => {
    ...
    if (statusIndex > -1) {
      statuses[statusIndex].status = newStatus as SiteTestCaseStatus["status"]
    } else {
      statuses.push({ test_case_id: testCaseId, status: newStatus as SiteTestCaseStatus["status"] })
    }
  ...
  const siteTestCases = useMemo((): TestCase[] => {
    ...
    library.useCases
      .filter((uc: UseCase) => editedSite.use_case_ids.includes(uc.id))
      .forEach((uc: UseCase) => {
        uc.test_case_ids.forEach((id) => testCaseIds.add(id))
      })
    return library.testCases.filter((tc: TestCase) => testCaseIds.has(tc.id))
